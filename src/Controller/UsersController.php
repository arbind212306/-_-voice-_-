    ...
    public function redirecturlafterlogin() {
        ...
        switch ($user_role) {
            ...
            case '3':
                ...
                $userid = $session->write('userid', $id);
                $this->viewBuilder()->layout('');
                return $this->redirect(['controller' => 'Dashboard', 'action' => 'dashboard', $id]);
                break;
            case '4':
                $session = $this->request->session();
...
// die();
                            ...
                            foreach($sheetData as $k => $usersdata){
                                ...
                                $userssdata->user_type = (empty($usersdata['A'])) ? 'NULL' : $usersdata['A'];
								   $userssdata->name = (empty($usersdata['B'])) ? 'NULL' : $usersdata['B'];
								      $userssdata->username = (empty($usersdata['C'])) ? 'NULL' : $usersdata['C'];
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Users->save($user)) {
				 ...
				 $email = new Email();
                    ...
                    $email->from(['Sanjeev.Yadav@quatrro.com' => 'MyVoice'])
                  ->to($user_email)
