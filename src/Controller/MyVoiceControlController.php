        ...
        if ($this->request->is('post')) {
            ...
            if ($usersTable->save($usersdata)) {
                ...
                $newemail->from(['learning@quatrro.com' => 'MyVoice'])
                        ...
                        ->emailFormat('html')
                        ->subject('Myvoice : Complaint rejected    ')
                        ->send();
...
INNER JOIN users as u  on u.id=c.assigned_to where c.assigned_to!= '' AND c.complaint_id=$id ";
        ...
        if ($this->request->is('post')) {
            ...
            if ($role_insert->save($role_data)) {
               ...
               $complaint_code= $this->Users->find()->where(['Users.id' => $complaint_id])->toArray();
                $complaint_user_code= $complaint_code[0]->complaint_id;
				
                $newemail = new Email();
                $newemail->viewVars(['name' => $assign_user_name, 'complaint_id' =>  $complaint_user_code]);
                $newemail->from(['learning@quatrro.com' => 'MyVoice'])
...
//            die;
            ...
            if (empty($tmpData)) {
                ...
                if ($panelTable->save($panel)) {
                    ...
                    $assign_user_email = $assign_role_user_details[0]->email;
                    $assign_user_name = $assign_role_user_details[0]->name;
					$assign_role_complaint_id = $this->Users->find()->where(['Users.id' =>$complaint_id])->toArray();
                    $individual_accuser_complaint_code = $assign_role_complaint_id[0]->complaint_id;
                    ...
                    $newemail->from(['learning@quatrro.com' => 'MyVoice'])
                            ...
                            ->emailFormat('html')
                            ->subject("Movice : New complain assigned ")
                            ->send();
    ...
    public function ComplaintDetailsSubmitForm($id = null) {
        ...
        if ($this->request->is('post')) {
            ...
            $userTable->save($ent);
             $individual_accuser_record = $this->Users->find()->where(['Users.id' => $id])->toArray();
			$complaint_id_user_name=$individual_accuser_record[0]->name;
			$complaint_id_user_email=$individual_accuser_record[0]->email;
			 $individual_accuser_complaint_code=$individual_accuser_record[0]->complaint_id;
			
          $newemail = new Email();
          $newemail->viewVars(['name' => $complaint_id_user_name, 'complaint_id' => $individual_accuser_complaint_code]);
           $newemail->from(['learning@quatrro.com' => 'MyVoice'])
                   ->to($complaint_id_user_email)

                  ->template('complaintCloseMail', 'mail')
                  ->emailFormat('html')
                   ->subject("Complaint $individual_accuser_complaint_code has been Closed ")
                  ->send();
...
//            }
    ...
    public function report() {
        ...
        if($this->request->is('get')) {
            ...
            $work_location = $this->request->query('location');
            if(!empty($category)){
                $tmp_con1 = ['Category.id' => $category];
                $where[] = $tmp_con1;
            }
            if(!empty($severity)){
                $tmp_con1 = ['severity LIKE ' => "%" . $severity . "%"];
                $where[] = $tmp_con1;
            }
            if(!empty($city)){
                $tmp_con1 = ['city LIKE ' => "%" . $city . "%"];
                $where[] = $tmp_con1;
            }
            if(!empty($case_status)){
                $tmp_con1 = ['Users.status' => $case_status];
                $where[] = $tmp_con1;
            }
